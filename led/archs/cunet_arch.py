...
class CUNetArch(nn.Module):
    ...
    def ratio_map_encoding(self, y):
        ...
        r = jt.exp(-((r - y) ** 2) / (2 * sigma * sigma)) / (math.sqrt(2 * math.pi) * sigma)
        r = jt.multiply(r, 1 / y)

        return r


    def execute(self, x, y, if_train=True):
