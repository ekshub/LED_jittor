...
# Jittor doesn't support custom Function with save_for_backward
# Use a simple LayerNorm implementation instead
class LayerNorm2d(nn.Module):
    def __init__(self, channels, eps=1e-6):
        super(LayerNorm2d, self).__init__()
        self.weight = nn.Parameter(jt.ones(channels))
        self.bias = nn.Parameter(jt.zeros(channels))
        self.eps = eps

    def execute(self, x):
        # Simple LayerNorm for 2D (N, C, H, W)
        N, C, H, W = x.shape
        # Compute mean and variance across channel dimension
        mu = x.mean(dim=1, keepdims=True)
        var = ((x - mu) ** 2).mean(dim=1, keepdims=True)
        # Normalize
        x_norm = (x - mu) / jt.sqrt(var + self.eps)
...
class NAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        self.beta = nn.Parameter(jt.zeros((1, c, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(jt.zeros((1, c, 1, 1)), requires_grad=True)
