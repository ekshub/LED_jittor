...
from led.utils.process import load_CRF, raw2rgb_torch, raw2rgb_torch_grad
from led.data.hdr_util import BlendMertens
...
def gamma_correct(linear, eps=None):
    ...
    srgb0 = 323 / 25 * linear
    srgb1 = (211 * torch.maximum(jt.array(eps), linear)**(5 / 12) - 11) / 200
...
def gamma_expansion(srgb, eps=None):
    if eps is None:
        eps = torch.finfo(torch.float32).eps
    linear0 = 25 / 323 * srgb
    linear1 = torch.maximum(jt.array(eps), ((200 * srgb + 11) / (211)))**(12 / 5)
...
def load_network(net, load_path, strict=True, param_key='params'):
    ...
    Args:
        ...
        param_key (str): The parameter key of loaded network. If set to
            ...
            Default: 'params'.
    """
    load_net = jt.load(load_path, map_location=lambda storage, loc: storage)
...
class IlluminanceCorrect(nn.Module):
    ...
    def execute(self, predict, source):
...
class RatioMapEstimatorModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if 'noise_g' in opt:
            ...
            jt.save(self.noise_g.state_dict(), dump_path)
    ...
    def feed_data(self, data):
        ...
        fused_raw = jt.clamp(exposure_fusion_from_raw(mexp_lq, self.wb, self.ccm, self.blend_merten), 1e-8)
        ...
        with jt.no_grad():
            ...
            lq_clean_patch, lq_im_patch, curr_metadata = self.noise_g(self.lq_clean, scale, self.ratio_all, if_clip = False)

        # # add noise
        # lq_im_patch = jt.clamp(self.lq_clean , min=0) * (16383 - 512) / self.ratio_all
        # im_patch, noise1 = self.noise_g(lq_im_patch)
    ...
    def get_current_visuals(self, isp=True, save_img=False):
        ...
        if isp:
            ...
            if save_img:
                ...
                if not self.calculate_metric_in_batch:
                    out_dict['result_srgb'] = out_dict['result_srgb'].numpy()
        if not self.calculate_metric_in_batch:
            out_dict['lq'] = out_dict['lq'].numpy()
            out_dict['result'] = out_dict['result'].numpy()
            out_dict['gt'] = out_dict['gt'].numpy()
        return out_dict
...
class RAWDenoiserModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        with jt.no_grad():
            scale = self.white_level - self.black_level
            lq_clean_patch, lq_im_patch, curr_metadata = self.noise_g(self.lq_clean, scale, self.ratio_all)
