...
from led.data.part_enhance import *


@DATASET_REGISTRY.register()
class EstimatorHDRRAWDataset(Dataset):
    ...
    def depack_meta(meta_path, postfix='npz', to_tensor=True):
        ...
        return (im - black_level) / (white_level - black_level), \
                wb, ccm, black_level, white_level, shape_ori
    ...
    def depack_meta_gt(meta_path, postfix='npz', to_tensor=True):
        ...
        return im, \
                wb, ccm, black_level, white_level


    def randint_fn(self, *range):
        if self.engine == 'torch':
            return jt.randint(*range, shape=(1,)).item()
        else:
            return np.random.randint(*range)

    def flip_fn(self, x, dim):
        if self.engine == 'torch':
            return jt.flip(x, dims=[dim])
        else:
    ...
    def __getitem__(self, index):
        ...
        if not self.load_in_mem:
            lq_im, lq_wb, lq_ccm, lq_black_level, lq_white_level = self.depack_meta(lq_path, self.postfix)
            gt_im, gt_wb, gt_ccm, gt_black_level, gt_white_level = self.depack_meta_gt(gt_path, self.postfix)
        else:
            lq_im, lq_wb, lq_ccm, lq_black_level, lq_white_level = self.lqs[lq_path]
            gt_im, gt_wb, gt_ccm, gt_black_level, gt_white_level = self.gts[gt_path]
        ...
        if self.opt['crop_size'] is not None:
            ...
            if self.opt['phase'] == 'train':
                h_start = jt.randint(0, H - crop_size, shape=(1,))[0]
                w_start = jt.randint(0, W - crop_size, shape=(1,))[0]
            else:
                # center crop
        ...
        ratio_all = ratio
        im_ratio = (gt_im_patch.mean() // (lq_im_patch.mean() + 1e-8)).clip(1, ratio_all)
        lq_im_patch = gt_im_patch / im_ratio
        ratio = ratio_all / im_ratio
        ...
        ratiomap_output = jt.mean(map_ratio, dim=0, keepdim=True).unsqueeze(0) * addmap
        
        lq_ev0_non_zero = lq_im_patch * im_ratio
        exposures = [
            jt.clamp(lq_ev0_non_zero, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 200, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 120, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 60, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 32, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 16, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 8, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 4, min=0, max=1),
            jt.clamp(lq_ev0_non_zero / 2, min=0, max=1),
        ]

        exposures_gt = jt.stack(exposures)
        lq_nonoise = lq_im_patch * im_ratio
        gt_im_patch = jt.clamp(gt_im_patch, min=0)
        ...
        return {
            ...
            'black_level': gt_black_level,
            'white_level': gt_white_level,
            'lq_path': lq_path,
            'gt_path': gt_path,
            'intact': True
        }
...
@DATASET_REGISTRY.register()
class DenoiserHDRRAWDataset(Dataset):
    def __init__(self, opt) -> None:
        super().__init__()
        self.opt = opt
        self.engine = self.opt.get('engine', 'torch')
