...
from jittor.nn import interpolate, pad, conv2d

class ClipBase(nn.Module):
    def __init__(self, clip=False) -> None:
        super().__init__()
        self._clip_func = jt.clamp if clip else diff_clamp
...
def get_laplacian_kernel2d(kernel_size, *, device = None, dtype = jt.float32):
    ky, kx = kernel_size
    kernel = jt.ones((ky, kx), dtype=dtype)
    mid_x = kx // 2
    mid_y = ky // 2
...
def diff_clamp(x, _min, _max, k=1e-3):
    x = jt.minimum(x - _max, (x - _max) * k) + _max
    x = jt.maximum(x - _min, (x - _min) * k) + _min
    return x
...
class BlendMertens(nn.Module):
    ...
    def get_weight(self, x):
        ...
        c_weight = jt.abs(laplacian_x)
        c_weight = c_weight ** self._contrast_weight

        # saturation
        s_weight = jt.std(x, dim=-3, keepdims=True)
        s_weight = s_weight ** self._saturation_weight

        # exposure
        sig = 0.2
        e_weight = jt.exp(-jt.pow(x - 0.5, 2) / (2 * sig * sig))
        r_w, g_w, b_w = jt.chunk(e_weight, 3, dim=-3)
    ...
    def execute(self, *data):
        ...
        weight_sum = jt.sum(weights, 0, keepdim=True)
