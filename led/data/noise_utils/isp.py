...
def random_ccm(batch_size, device='cuda'):
    ...
    xyz2cams = jt.array(xyz2cams, device=device).unsqueeze(0)
    weights = jt.rand((batch_size, num_ccms, 1, 1), device=device) * 1e8 + 1e-8
    weights_sum = jt.sum(weights, dim=1)
    xyz2cam = jt.sum(xyz2cams * weights, dim=1) / weights_sum

    # Multiplies with RGB -> XYZ to get RGB -> Camera CCM.
    rgb2xyz = jt.array([[0.4124564, 0.3575761, 0.1804375],
                            [0.2126729, 0.7151522, 0.0721750],
                            [0.0193339, 0.1191920, 0.9503041]], device=device)
    rgb2cam = xyz2cam @ rgb2xyz

    # Normalizes each row.
    rgb2cam = rgb2cam / jt.sum(rgb2cam, dim=-1, keepdims=True)
    return rgb2cam
...
def random_gain(batch_size, device='cuda'):
    ...
    rgb_gain = 1.0 / (jt.randn((batch_size), device=device) * 0.1 + 0.8)

    # Red and blue gains represent white balance.
    red_gain = jt.rand((batch_size), device=device) * (2.4 - 1.9) + 1.9
    blue_gain = jt.rand((batch_size), device=device) * (1.9 - 1.5) + 1.5
...
def gamma_expansion(image):
    ...
    return torch.maximum(image, jt.array(1e-8, device=image.device)) ** 2.2


def gamma_compression(image, gamma=2.2):
    """Converts from linear to gamma space."""
    # Clamps to prevent numerical instability of gradients near zero.
    return torch.maximum(image, jt.array(1e-8, device=image.device)) ** (1.0 / gamma)
...
def apply_gains(bayer_images, rgb_gains, red_gains, blue_gains, in_type='rgbg'):
    ...
    if in_type == 'rggb':
        gains = jt.concat([red_gains, green_gains, green_gains, blue_gains], dim=-3) * rgb_gains
    elif in_type == 'rgbg':
        gains = jt.concat([red_gains, green_gains, blue_gains, green_gains], dim=-3) * rgb_gains
...
def safe_invert_gains(image, rgb_gain, red_gain, blue_gain):
    ...
    gray = jt.mean(image, dim=-3, keepdims=True)
    inflection = 0.9
    mask = (torch.maximum(gray - inflection, jt.array(0.0, device=gray.device)) / (1.0 - inflection)) ** 2.0
...
def mosaic(image, out_type):
    ...
    b = image[..., 2:3, 1::2, 1::2]
    if out_type == 'rgbg':
        out = jt.concat([r, gr, b, gb], dim=-3)
    elif out_type == 'rggb':
        out = jt.concat([r, gr, gb, b], dim=-3)
    return out
...
def half_size_demosaic(bayer_images, in_type):
    ...
    if in_type == 'rggb':
        ...
        gb = bayer_images[..., 3:4, :, :]
    g = (gr + gb) / 2
    linear_rgb = jt.concat([r, g, b], dim=-3)
    return linear_rgb
...
def demosaic(bayer_images, in_type='rgbg'):
    ...
    green = depth_to_space(jt.stack(green_planes, dim=1), 2)

    blue = torch.flipud(torch.fliplr(blue))
    blue = bilinear_interpolate(blue, shape)
    blue = torch.flipud(torch.fliplr(blue))

    rgb_images = jt.concat([red, green, blue], dim=1)
    if T is not None:
...
def reverse_isp(srgb, ccm, gains, scale, out_type='rgbg', safe_invert_gain=False):
    ...
    linear_rgb = jt.clamp(linear_rgb, 0.0, 1.0)
