...
def _uniform_batch(min_, max_, shape=(1,), device='cpu'):
    return jt.rand(shape, device=device) * (max_ - min_) + min_

def _normal_batch(scale=1.0, loc=0.0, shape=(1,), device='cpu'):
    return jt.randn(shape, device=device) * scale + loc
...
class CalibratedNoisyPairGenerator(nn.Module):
        ...
        def to_tensor_dict(p_dict):
            for k, v in p_dict.items():
                if isinstance(v, list) or isinstance(v, Number):
                    p_dict[k] = nn.Parameter(jt.array(v, device=self.device), False)
    ...
    def sample_overall_system_gain(self, batch_size, for_video):
        ...
        log_K_max = jt.log(self.current_camera_params['Kmax'])
        log_K_min = jt.log(self.current_camera_params['Kmin'])
    ...
    def sample_read_sigma(self):
        ...
        return jt.exp(sample).reshape(self.log_K.shape)
    ...
    def sample_row_sigma(self):
        ...
        mu = self.log_K.squeeze() * slope + bias
        sample = _normal_batch(sigma, mu, (self.cur_batch_size,), self.device)
        return jt.exp(sample).reshape(self.log_K.squeeze(-3).shape)
    ...
    def sample_color_bias(self, batch_size, for_video):
        ...
        index = ((jt.exp(self.log_K.squeeze()) - self.current_k_range[0]) // i_range).int()
    ...
    def add_noise(img, noise, noise_params, if_clip=True):
        ...
        img = img * ratio
        if if_clip:
            img = jt.clamp(img, max=1.0)
        return img

    @jt.no_grad()
    def execute(self, img, scale, ratio, vcam_id=None, if_clip=True):
        b = img.size(0)
        for_video = True if img.dim() == 5 else False # B, T, C, H, W
        self.index = vcam_id if vcam_id is not None else None

        if if_clip:
            img_gt = jt.clamp(img, 0, 1)
        else:
            img_gt = img
        tail = [1 for _ in range(img.dim() - 1)]
...
class VirtualNoisyPairGenerator(nn.Module):
    ...
    def sample_virtual_cameras(self):
        ...
        # overall system gain
        self.k_range = jt.array(self.param_ranges['K'], device=self.device)
        ...
        if 'c' in self.noise_type:
            ...
            color_bias_sigmas = self.split_range_overlap(self.color_bias_count,
                                                         ...
                                                         overlap=0.1)
            self.color_biases = jt.array(np.array([
                [
                    random.uniform(*self.param_ranges['ColorBias']['bias']) + \
                        jt.randn(4).numpy() * random.uniform(*color_bias_sigmas[i]).numpy().numpy()
    ...
    def uniform_range(splits, range_, shuffle=True, device='cuda'):
        ...
        return jt.array(results, device=device)
    ...
    def sample_overall_system_gain(self, batch_size, for_video):
        ...
        log_K_max = jt.log(self.k_range[1])
        log_K_min = jt.log(self.k_range[0])
        log_K = _uniform_batch(log_K_min, log_K_max, (batch_size, 1, 1, 1), self.device)
        if for_video:
            log_K = log_K.unsqueeze(-1)
        self.log_K = log_K
    ...
    def sample_color_bias(self, batch_size, for_video):
        ...
        index = ((jt.exp(self.log_K.squeeze()) - self.k_range[0]) // i_range).int()
    ...
    def add_noise(img, noise, noise_params):
        ...
        img /= scale
        img = img * ratio
        return jt.clamp(img, max=1.0)

    @jt.no_grad()
    def execute(self, img, scale, ratio, vcam_id=None):
        b = img.size(0)
        for_video = True if img.dim() == 5 else False # B, T, C, H, W
        self.current_camera = vcam_id * jt.ones((b,), dtype=torch.long, device=self.device) \
    ...
    def json_dict(self):
        ...
        for i in range(self.virtual_camera_count):
            json_dict[f'IC{i}']['Kmin'] = self.k_range[0].numpy().numpy().tolist()
            json_dict[f'IC{i}']['Kmax'] = self.k_range[1].numpy().numpy().tolist()

        if 'g' in self.noise_type or 't' in self.noise_type:
            read_log_key = 'G' if 'g' in self.noise_type else 'TL'
            for i in range(len(self.read_slopes)):
                json_dict[f'IC{i}'][f'{read_log_key}_slope'] = self.read_slopes[i].numpy().numpy().tolist()
                json_dict[f'IC{i}'][f'{read_log_key}_bias'] = self.read_biases[i].numpy().numpy().tolist()
                json_dict[f'IC{i}'][f'{read_log_key}_sigma'] = self.read_sigmas[i].numpy().numpy().tolist()
                if read_log_key == 'TL':
                    json_dict[f'IC{i}'][f'{read_log_key}_lambda'] = self.tukey_lambdas[i].numpy().numpy().tolist()

        if 'r' in self.noise_type:
            for i in range(len(self.row_slopes)):
                json_dict[f'IC{i}']['Row_slope'] = self.row_slopes[i].numpy().numpy().tolist()
                json_dict[f'IC{i}']['Row_bias'] = self.row_biases[i].numpy().numpy().tolist()
                json_dict[f'IC{i}']['Row_sigma'] = self.row_sigmas[i].numpy().numpy().tolist()

        if 'c' in self.noise_type:
            for i in range(len(self.color_biases)):
                json_dict[f'IC{i}']['CB_biases'] = self.color_biases[i].numpy().numpy().tolist()
