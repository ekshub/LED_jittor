...
def _pack_bayer(raw):
    ...
    out = jt.concat((raw[..., None, 0:h:2, 0:w:2], # R
...
def row_noise(x, scale, loc=0):
    if x.dim() == 4:
        B, _, H, W = x.shape
        noise = (jt.randn((B, H * 2, 1), device=x.device) * scale + loc).repeat((1, 1, W * 2))
        return _pack_bayer(noise)
    elif x.dim() == 5:
        B, T, _, H, W = x.shape
        noise = (jt.randn((B, T, H * 2, 1), device=x.device) * scale + loc).repeat((1, 1, 1, W * 2))
        return _pack_bayer(noise)
    else:
