...
class EstimatorHighlightGenerator:
    ...
    def gaussian_kernel(self, distance, radius, max_gain, boundary_gain):
        return max_gain * jt.exp(-(distance ** 2) / (2 * (radius ** 2)))
    
    def inverse_square_exponential_kernel(self, distance, radius, max_gain, boundary_gain):
        epsilon = 1.0
        beta = 1.0
        return (max_gain / ((distance / radius) ** 2 + epsilon)) * jt.exp(-beta * (distance / radius))
    ...
    def generate_highlight(self, tensor):
        ...
        mask = jt.ones((H, W), dtype=torch.uint8)
        addmap = jt.zeros((H, W))
        ...
        region_gain = jt.rand(1).item() * self.max_gain
        region_gain_matrix = jt.ones((H, W))

        if region_type == 0:
            y_grid, x_grid = torch.meshgrid(jt.arange(H), jt.arange(W), indexing="ij")
            ...
            angles = jt.linspace(0, 2 * math.pi, num_sides + 1)
            ...
            region_mask = jt.zeros((H, W), dtype=torch.bool)
        ...
        final_image = jt.clamp(final_image, 0, 1)
...
class DenoiserHighlightGenerator:
    ...
    def gaussian_kernel(self, distance, radius, max_gain, boundary_gain):
        ...
        return max_gain * jt.exp(-(distance ** 2) / (alpha * (radius ** 2)))
    ...
    def torch_line_fill(self, mask, x1, y1, x2, y2):
        ...
        for i in range(x1, x2):
            ymin = jt.array(min(np.floor(y1 + b * y1 * np.sin(i/a*np.pi)), H-2)).int()
            ymax = jt.array(min(np.floor(y2 + c * y2 * np.cos(i/a*np.pi)), H-2)).int()
            mask[ymin:ymax, i] = 1
    ...
    def generate_highlight(self, tensor):
        ...
        for _ in range(num_regions):
            ...
            region_mask_np = region_mask.detach().numpy().numpy()
